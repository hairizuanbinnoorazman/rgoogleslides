...
#' @param shapeType A list that contains the type of shape that is to be drawn
...
#' @export
build_create_shape <- function(shapeType = NULL, pageElementPropert = NULL, objectId = NULL){
  # Check to see if there is any requests_list provided. Else, reinitialize it.
  if(is.null(requests_list)){
    warning("No/Invalid request_list provided. request_list reinitialized")
    requests_list <- list()
  ...
  while(iterator <= length(pageElementProperty)){
    create_shape_list <- list(createImage = list(elementProperties = pageElementProperty[[iterator]],
